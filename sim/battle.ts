...
export class Battle {
	...
	singleEvent(
		...
		if (this.log.length - this.sentLogPos > 100000000) {
	...
	maybeTriggerEndlessBattleClause(
		...
		if (
			(this.turn >= 5000 && this.turn % 1000 === 0) || // every 1000 turns past turn 5000,
			(this.turn >= 10000 && this.turn % 100 === 0) || // every 100 turns past turn 10000,
			(this.turn >= 19000 && this.turn % 10 === 0) || // every 10 turns past turn 19000,
			this.turn >= 19900 // every turn past turn 19900
		) {
			const turnsLeft = 20000 - this.turn;
			const turnsLeftText = (turnsLeft === 1 ? `1 turn` : `${turnsLeft} turns`);
			this.add('bigerror', `You will auto-tie if the battle doesn't end in ${turnsLeftText} (on turn 20000).`);
		}
