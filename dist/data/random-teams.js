...
class RandomTeams {
  ...
  shouldCullAbility(ability, types, moves, abilities, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    switch (ability) {
      ...
      case "Swift Swim":
        return abilities.has("Intimidate") || !moves.has("raindance") && !teamDetails.rain || species.id === "drednaw" && moves.has("crunch");
      case "Synchronize":
  ...
  getAbility(types, moves, abilities, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (species.id === "swampert" && !counter.get("Water") && !moves.has("flipturn"))
      return "Damp";
    if (species.id === "thundurus" && (role === "Offensive Protect" || moves.has("terablast")))
      return "Defiant";
    ...
    if (isDoubles) {
      ...
      if (species.id === "farigiraf")
        return "Armor Tail";
      if (["carbink", "dragapult", "regirock", "tentacruel"].includes(species.id))
        return "Clear Body";
  ...
  getItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
    ...
    if (moves.has("chillyreception") || role === "Fast Support" && [...PIVOT_MOVES, "defog", "mortalspin", "rapidspin"].some((m) => moves.has(m)) && !types.includes("Flying") && ability !== "Levitate")
      return "Heavy-Duty Boots";
    if (species.id === "garchomp" && role === "Fast Support" || ability === "Regenerator" && (role === "Bulky Support" || role === "Bulky Attacker") && species.baseStats.hp + species.baseStats.def >= 180 && this.randomChance(1, 2) || ability !== "Regenerator" && !counter.get("setup") && counter.get("recovery") && this.dex.getEffectiveness("Fighting", species) < 1 && species.baseStats.hp + species.baseStats.def > 200 && this.randomChance(1, 2))
      return "Rocky Helmet";
    if (moves.has("outrage") && this.randomChance(1, 2))
      return "Lum Berry";
