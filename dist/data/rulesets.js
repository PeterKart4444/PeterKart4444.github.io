...
(You have sketched ${moveName} ${count} times.)`);
    ...
    onModifySpecies(species, target, source, effect) {
      ...
      if (target) {
        if (this.toID(target.set.item) === "lightclay")
          tier = "rubl";
        if (this.toID(target.set.item) === "damprock")
          tier = "publ";
        if (this.toID(target.set.item) === "heatrock")
          tier = "publ";
      }
  ...
  hackmonsformelegality: {
    ...
    onChangeSet(set, format, setHas, teamHas) {
      let species = this.dex.species.get(set.species);
      if ((species.natDexTier === "Illegal" || species.forme.includes("Totem")) && !["Floette-Eternal", "Greninja-Ash", "Xerneas-Neutral"].includes(species.name) && !this.ruleTable.has(`+pokemon:${species.id}`)) {
  ...
  dynamaxmod: {
    ...
    onBegin() {
      ...
      this.add("rule", "Dynamax Mod: You can dynamax (coded by Opacous)");
    }
  },
  peterkartclause: {
    effectType: "ValidatorRule",
    name: "PeterKart Clause",
    desc: "Bans the moves Beat Up and Last Respects",
    banlist: ["Beat Up", "Last Respects"],
    onBegin() {
      this.add("rule", "PeterKart Clause: Beat Up and Last Respects are banned");
    }
  },
