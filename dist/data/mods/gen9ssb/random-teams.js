...
class RandomStaffBrosTeams extends import_random_teams.default {
  randomStaffBrosTeam(options = {}) {
    ...
    while (pool.length && team.length < this.maxTeamSize) {
      ...
      const ssbSet = this.dex.deepClone(ssbSets[name]);
      if (ssbSet.skip)
        continue;
