  ...
  */
  ...
  recharge: {
    ...
    onModifyMove(move, source, target) {
      ...
      if (move.heal)
        move.heal[0] = 50 + 3 * fntAllies.length;
    },
    onTryMove() {
      this.attrLastMove("[still]");
    },
  ...
  stormshelter: {
    ...
    onHit(pokemon) {
      ...
      this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|Ope! Wrong button, sorry.`);
      const unloweredStat = this.sample(Object.keys(pokemon.boosts).filter((x) => x !== "evasion"));
      for (const boost in boosts) {
        if (boosts[boost] >= 6 && maxBoostIDs.includes(boost) || boost === unloweredStat)
          continue;
        this.boost({ [boost]: -1 }, pokemon);
      }
    },
  ...
  plagiarism: {
    ...
    onHit(target, source, m) {
      const sigMoveName = import_random_teams.ssbSets[(target.illusion || target).name].signatureMove;
      const move = this.dex.getActiveMove(sigMoveName);
      if (!target || this.queue.willSwitch(target) || target.beingCalledBack || move.flags["failcopycat"] || move.noSketch) {
  ...
  "1000gears": {
    ...
    onHit(target, pokemon, move) {
      ...
      pokemon.level += 5;
      pokemon.details = pokemon.species.name + (pokemon.level === 100 ? "" : ", L" + pokemon.level) + (pokemon.gender === "" ? "" : ", " + pokemon.gender) + (pokemon.set.shiny ? ", shiny" : "");
    ...
    onHit(target, source, move) {
      if (source.positiveBoosts() < target.positiveBoosts()) {
        ...
        for (stat2 in source.boosts) {
          if (stat2 === "accuracy" || stat2 === "evasion")
            continue;
  ...
  perfectmimic: {
    ...
    onDisableMove(pokemon) {
      if (pokemon.lastMove?.id === "perfectmimic")
        pokemon.disableMove("perfectmimic");
    },
    condition: {
      duration: 1,
      onStart(target) {
      ...
      onSourceAfterMove(source, target) {
        if (source === this.effectState.target || target !== this.effectState.target)
          return;
        if (!source.hp || !this.effectState.move)
          return;
  ...
  gmaxsteelsurge: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.hasAbility("eternalgenerator"))
          return;
