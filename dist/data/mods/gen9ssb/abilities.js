  ...
  */
  ...
  onemore: {
    ...
    onHit(target, source, move) {
      ...
      if (move.category === "Status" || hitData.typeMod <= 0 || !hitData.crit)
        return;
      if (!move.secondaries)
        move.secondaries = [];
  ...
  cascade: {
    ...
    onEmergencyExit(target) {
      if (target.volatiles["sigilsstorm"]?.lostFocus) {
        ...
        if (this.effectState.cascade === void 0) {
          this.effectState.cascade = 1;
        } else {
          this.effectState.cascade = 0;
        }
        this.add(`c:|${(0, import_scripts.getName)("ausma")}|uuuuuuuuuuuuuuuuuuuuuuuuugggggghhhhhhhh [dizzy sound effect] sec bitte`);
    ...
    onSwitchIn() {
      if (this.effectState.cascade) {
        ...
        this.add(`c:|${(0, import_scripts.getName)("ausma")}|ok i got my coffee yall mfs r about to face the wrath of Big Stall\u2122`);
        this.effectState.cascade = 0;
      }
  ...
  painfulexit: {
    ...
    onBeforeSwitchOut(pokemon) {
      if ((0, import_scripts.enemyStaff)(pokemon) === "Mad Monty") {
        this.add(`c:|${(0, import_scripts.getName)("Breadstycks")}|Welp`);
      } else {
        this.add(`c:|${(0, import_scripts.getName)("Breadstycks")}|Just kidding!! Take this KNUCKLE SANDWICH`);
      }
    ...
    onDamage(damage, target, source, effect) {
      if (effect.id === "recoil") {
        ...
        if (this.activeMove.id !== "struggle") {
          if (!source.hasType(this.activeMove.type))
            this.heal(damage);
          return null;
        }
      }
  ...
  illusionmaster: {
    ...
    onEnd(pokemon) {
      if (pokemon.illusion) {
        ...
        if (this.dex.species.get(disguisedAs).exists || this.dex.moves.get(disguisedAs).exists || this.dex.abilities.get(disguisedAs).exists || disguisedAs === "blitz") {
          disguisedAs += "user";
        }
    ...
    onWeatherChange(pokemon) {
      ...
      if (pokemon.isActive && forme) {
        ...
        const sketchIndex = Math.max(
          pokemon.moves.indexOf("solarbeam"),
          pokemon.moves.indexOf("thunder"),
          pokemon.moves.indexOf("auroraveil")
        );
        if (sketchIndex < 0)
          return;
    ...
    onDamage(damage, target, source, effect) {
      ...
      if (target.set && !target.set.shiny) {
        if (target.m.damageTaken >= 420) {
          ...
          const details = target.species.name + (target.level === 100 ? "" : ", L" + target.level) + (target.gender === "" ? "" : ", " + target.gender) + (target.set.shiny ? ", shiny" : "");
  ...
  adaptiveengineering: {
    ...
    onResidual(source) {
      if (source === void 0 || source.foes() === void 0 || source.foes()[0] === void 0)
        return;
  ...
  wildgrowth: {
    ...
    onModifyMovePriority: -1,
    onAfterMove(source, target, move) {
      if (target.hasType("Grass") || target.hasAbility("Sap Sipper") || !move.hit)
        return null;
