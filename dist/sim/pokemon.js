  ...
  constructor(set, side) {
    this.getDetails = () => {
      ...
      if (this.illusion) {
        ...
        const illusionDetails = displayedSpeciesName + (level === 100 ? "" : ", L" + level) + (this.illusion.gender === "" ? "" : ", " + this.illusion.gender) + (this.illusion.set.shiny ? ", shiny" : "");
    ...
    set.level = this.battle.clampIntRange(set.adjustLevel || set.level || 100, 1, 999999999);
  ...
  */
  ...
  formeChange(speciesId, source = this.battle.effect, isPermanent, message) {
    ...
    if (isPermanent && (!source || !["disguise", "iceface"].includes(source.id))) {
      ...
      this.baseAbility = (0, import_dex.toID)(species.abilities["0"]);
    }
    if (this.terastallized) {
      this.knownType = true;
      this.apparentType = this.terastallized;
