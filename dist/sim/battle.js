...
class Battle {
  ...
  singleEvent(eventid, effect, state, target, source, sourceEffect, relayVar, customCallback) {
    ...
    if (this.log.length - this.sentLogPos > 1e8) {
  ...
  maybeTriggerEndlessBattleClause(trappedBySide, stalenessBySide) {
    ...
    if (this.turn >= 5e3 && this.turn % 1e3 === 0 || this.turn >= 1e4 && this.turn % 100 === 0 || this.turn >= 19e3 && this.turn % 10 === 0 || // every 10 turns past turn 19000,
    this.turn >= 19900) {
      const turnsLeft = 2e4 - this.turn;
