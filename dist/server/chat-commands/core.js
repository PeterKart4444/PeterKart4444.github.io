...
const commands = {
  ...
  invite(target, room, user) {
    ...
    if (!pmTarget) {
      ...
      const targetRoom2 = targetRoomid ? Rooms.search(targetRoomid) : room;
      if (!targetRoom2)
        return this.errorReply(this.tr`The room "${targetRoomid}" was not found.`);
      return this.parse(`/pm ${targetUser.name}, /invite ${targetRoom2.roomid}`);
    }
  ...
  status(target, room, user, connection, cmd) {
    ...
    if (!target)
      return this.errorReply(this.tr`Your status contains a banned word.`);
  ...
  back(target, room, user) {
    if (user.statusType === "online")
      return this.errorReply(this.tr`You are already marked as back.`);
  ...
  hiderank(target, room, user, connection, cmd) {
    ...
    if (!Users.Auth.isValidSymbol(group) || !(group in Config.groups) || group === Users.SECTIONLEADER_SYMBOL && !(Users.globalAuth.sectionLeaders.has(user.id) || user.can("bypassall"))) {
...
- ${result.slice(1).replace(/\n/g, "\n- ")}`);
  ...
  help(target, room, user) {
    ...
    const getHelp = (namespace, cmds) => {
      ...
      if (curHandler?.isPrivate && !user.can(requiredPerm)) {
