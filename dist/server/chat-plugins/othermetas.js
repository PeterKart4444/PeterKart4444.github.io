...
function getMegaStone(stone, mod = "gen9") {
  ...
  if (!item.megaStone && !item.onPrimal && !item.forcedForme?.endsWith("Epilogue") && !item.forcedForme?.endsWith("Origin") && !item.name.startsWith("Rusted") && !item.name.endsWith("Mask"))
    return null;
  return item;
}
const commands = {
  ...
  mixandmega(target, room, user) {
    ...
    switch (stone.id) {
      ...
      default:
        ...
        if (forcedForme && (forcedForme.startsWith("Ogerpon") || forcedForme.endsWith("Origin") || forcedForme.endsWith("Epilogue"))) {
    ...
    if (mixedSpecies.types[0] === deltas.type) {
