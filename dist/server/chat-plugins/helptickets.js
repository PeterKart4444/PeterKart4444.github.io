...
function writeStats(line) {
  ...
  try {
    (0, import_lib.FS)(`logs/tickets/${month}.tsv`).appendSync(line + "\n");
  } catch (e) {
    if (e.code !== "ENOENT")
      throw e;
  }
...
class HelpTicket extends Rooms.SimpleRoomGame {
  ...
  static logTextResult(ticket) {
    ...
    void (0, import_lib.FS)(`logs/tickets/${date.slice(0, -3)}.jsonl`).append(JSON.stringify(entry) + "\n");
  }
  /**
    ...
    if (await (0, import_config_loader.checkRipgrepAvailability)()) {
      ...
      try {
        lines = await import_lib.ProcessManager.exec([
          `rg`,
          (0, import_lib.FS)(`logs/tickets/${date ? `${date}.jsonl` : ""}`).path,
          ...args
        ]);
      } catch (e) {
      ...
      const path = (0, import_lib.FS)(`logs/tickets/${date}.jsonl`);
      if (!path.existsSync()) {
...
async function getOpponent(link, submitter) {
  ...
  if (!room) {
    const replayUrl = (0, import_lib.Net)(`https://${Config.routes.replays}/${link.slice(link.indexOf("-") + 1)}.json`);
    try {
  ...
  if (battleRoom && battleRoom.type !== "chat") {
    ...
    for (const line of battleRoom.log.log) {
      if (line.startsWith("|switch|")) {
        ...
        monTable[slot].push({
          species,
          name: species === name ? void 0 : name
        });
      }
      if (line.startsWith("|player|")) {
        const [, , playerSlot, name] = line.split("|");
        playerTable[playerSlot] = toID(name);
      }
      for (const k in monTable) {
        const userid = playerTable[k];
        if (userid) {
...
const textTickets = {
  ...
  inappokemon: {
    ...
    async getReviewDisplay(ticket, staff, conn) {
      ...
      const battleRooms = links.map((r) => Rooms.get(r)).filter((room) => room?.battle);
...
|pagehtml|<div class="pad"><h2>${this.tr`Request help from global staff`}</h2><p>${this.tr`Please <button name="login" class="button">Log In</button> to request help.`}</p></div>`;
    ...
    async logs(query, user, connection) {
      ...
      if (!userid)
        return this.errorReply(`Specify a userid to view ticket logs for.`);
    ...
    stats(query, user, connection) {
      ...
      const rawTicketStats = (0, import_lib.FS)(`logs/tickets/${dateUrl}.tsv`).readIfExistsSync();
      if (!rawTicketStats)
      ...
      let buttonBar = "";
      if ((0, import_lib.FS)(`logs/tickets/${prevString}.tsv`).readIfExistsSync()) {
      ...
      if ((0, import_lib.FS)(`logs/tickets/${nextString}.tsv`).readIfExistsSync()) {
...
const commands = {
  ...
  helpticket: {
    ...
    create(target, room, user) {
      ...
      if (!user.named)
        return this.errorReply(this.tr`You need to choose a username before doing this.`);
      ...
      if (reportTargetType === "room") {
        ...
        if (targetID !== ticket.userid) {
          ...
          if (!commonBattles.length) {
            ...
            reportTargetInfo += commonBattles.map((roomid) => import_lib.Utils.html`<a href=/${roomid}>${roomid.replace(/^battle-/, "")}`);
          }
        }
      }
...
|deinit`);
    ...
    addnote(target, room, user) {
      ...
      if (!ticketId)
        return this.errorReply(`Specify the userid that created the ticket you want to mark.`);
      const ticket = tickets[ticketId];
      if (!ticket)
        return this.errorReply(`${ticketId} does not have an active ticket.`);
      if (ticket.resolved)
        return this.errorReply(`${ticketId}'s ticket has already been resolved.`);
      if (!note)
        return this.errorReply(`You must specify a note to add.`);
      if (!ticket.notes)
        ticket.notes = {};
    ...
    removenote(target, room, user) {
      ...
      if (!targetId)
        return this.errorReply(`Specify the userid that created the ticket you want to remove a note from.`);
      const ticket = tickets[targetId];
      if (!ticket || ticket.resolved)
        return this.errorReply(`${targetId} does not have a pending ticket.`);
      staff = toID(staff) || user.id;
      if (!ticket.notes)
        return this.errorReply(`${targetId}'s ticket does not have any notes.`);
...
|deinit|`);
    ...
    delete(target, room, user) {
      ...
      if (!ticket)
        return this.errorReply(this.tr`${target} does not have a ticket.`);
    ...
    logs(target, room, user) {
      ...
      if (!id)
        return this.errorReply(`Specify a userid.`);
    ...
    async private(target, room, user) {
      ...
      const logPath = (0, import_lib.FS)(`logs/chat/help-${userid}/${date.slice(0, -3)}/${date}.txt`);
      if (!await logPath.exists()) {
        return this.errorReply(`There are no logs for tickets from '${userid}' on the date '${date}'.`);
      }
      if (!await (0, import_lib.FS)(`logs/private/${userid}`).exists()) {
        await (0, import_lib.FS)(`logs/private/${userid}`).mkdirp();
      }
    ...
    async public(target, room, user) {
      ...
      const logPath = (0, import_lib.FS)(`logs/private/${userid}/${date}.txt`);
      if (!await logPath.exists()) {
        return this.errorReply(`There are no logs for tickets from '${userid}' on the date '${date}'.`);
      }
      const monthPath = (0, import_lib.FS)(`logs/chat/help-${userid}/${date.slice(0, -3)}`);
